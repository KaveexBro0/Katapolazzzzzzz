...
export default async function handler(req, res) {
  ...
  await client.query(
    ...
    [betAmount, payout, userId]
  );
  await client.query(
    `INSERT INTO games (userId, mode, betType, betAmount, result, payout) VALUES ($1, $2, $3, $4, $5, $6)`,
    [userId, 'Auto', betType, betAmount, dice WINS, payout]
  );
  const { rows } = await client.query(`SELECT balance FROM users WHERE id = $1`, [
    userId,
  ]);
  await client.end();

  res.json({ dice, won, payout, balance: rows[0].balance });
}